...
// Font look-up table
//===================
const unsigned int glyphSet[]PROGMEM = {0x0660, // 0x00 NUL  ....  ....  ....  ....
                                        0x0660, // 0x01 SOH  .##.  .##.  .##.  .##.
                                        0x0660, // 0x02 STX  .##.  .##.  .##.  .##.
                                        0x0660, // 0x03 ETX  ....  ....  ....  ....
...
//Shift frame and append new pattern
//==================================
ledState txtShiftAndAppend(ledState frame, unsigned int pattern) {
  //Local variables
  ledState patternFrame = pattern;
  
  //Shift frame in negative Y direction
  frame  = LED_STATE_UNSHIFT_Y(frame);

  //Insert pattern into slice Y3
  frame  |= (((0xF000 & patternFrame)          ) |  //C3
            ((0x0F00 & patternFrame) << ( 5*4)) |  //C7
            ((0x00F0 & patternFrame) << (10*4)) |  //C11
...
//Shift out LED content
//=====================
ledState txtShiftOut(ledState frame) {
  while (frame) {
    frame = txtShiftAndAppend(frame, 0);
  }
...
// Display text
//=============
ledState txtPrint (ledState frame, char *string) {
  //local variables
  unsigned char character;
  unsigned int  glyph;
    
  //Shift out previous LED content
  frame = txtShiftOut(frame);

  //Display string 
  while (character = *string++) {          //iterate through the string
    //Look up glyph 
    glyph = pgm_read_word_near(glyphSet + character);

    //Print character 
    //frame = glyph;
    //dispQueueFrames(frame, 4*FRAMERATE);

    frame = txtShiftAndAppend(frame, 0);      //append blanc slice
    frame = txtShiftAndAppend(frame, 0);      //append blanc slice
    frame = txtShiftAndAppend(frame, 0);      //append blanc slice
    frame = txtShiftAndAppend(frame, glyph);  //append pattern
  }
